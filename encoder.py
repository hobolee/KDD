...
class Encoder(nn.Module):
    ...
    def forward_by_stage(self, inputs, subnet, rnn):
        seq_number, batch_size, input_channel, height = inputs.size()
        inputs = torch.reshape(inputs, (-1, input_channel, height))
        inputs = subnet(inputs)
        inputs = torch.reshape(inputs, (seq_number, batch_size, inputs.size(1),
                                        inputs.size(2)))
        outputs_stage, state_stage = rnn(inputs, None, seq_len=12)
